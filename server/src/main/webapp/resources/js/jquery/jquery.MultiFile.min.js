...
window.jQuery && function(d) {
    ...
    d.fn.MultiFile = function(e) {
        ...
        d.fn.MultiFile.options.autoIntercept && (d.fn.MultiFile.intercept(d.fn.MultiFile.options.autoIntercept),
        d.fn.MultiFile.options.autoIntercept = null),
        this.not(".MultiFile-applied").addClass("MultiFile-applied").each(function() {
            ...
            0 < p.max || (p.max = (String(f.e.className.match(/\b(max|limit)\-([0-9]+)\b/gi) || [""]).match(/[0-9]+/gi) || [""])[0],
            0 < p.max ? p.max = String(p.max).match(/[0-9]+/gi)[0] : p.max = -1),
            p.max = new Number(p.max),
            p.accept = p.accept || f.E.attr("accept") || "",
            p.accept || (p.accept = f.e.className.match(/\b(accept\-[\w\|]+)\b/gi) || "",
            p.accept = new String(p.accept).replace(/^(accept|ext)\-/i, "")),
            p.maxsize = 0 < p.maxsize ? p.maxsize : f.E.data("maxsize") || 0,
            0 < p.maxsize || (p.maxsize = (String(f.e.className.match(/\b(maxsize|maxload|size)\-([0-9]+)\b/gi) || [""]).match(/[0-9]+/gi) || [""])[0],
            0 < p.maxsize ? p.maxsize = String(p.maxsize).match(/[0-9]+/gi)[0] : p.maxsize = -1),
            p.maxfile = 0 < p.maxfile ? p.maxfile : f.E.data("maxfile") || 0,
            0 < p.maxfile || (p.maxfile = (String(f.e.className.match(/\b(maxfile|filemax)\-([0-9]+)\b/gi) || [""]).match(/[0-9]+/gi) || [""])[0],
            0 < p.maxfile ? p.maxfile = String(p.maxfile).match(/[0-9]+/gi)[0] : p.maxfile = -1),
            ...
            1 < String(f.accept).length && (f.accept = f.accept.replace(/\W+/g, "|").replace(/^\W|\W$/g, ""),
            ...
            f.addSlave = function(u, m) {
                ...
                u.id = u.name = "",
                u.id = f.generateID(m),
                u.name = String(f.namePattern.replace(/\$name/gi, d(f.clone).attr("name")).replace(/\$id/gi, d(f.clone).attr("id")).replace(/\$g/gi, i).replace(/\$i/gi, m)),
                ...
                u.change(function(e, i, t) {
                    ...
                    d.each(l, function(e, i) {
                        ...
                        var l = i.name.replace(/^C:\\fakepath\\/gi, "")
                          , t = i.size;
                        f.accept && l && !l.match(f.rxAccept) && (o[o.length] = a(f.STRING.denied),
                        f.trigger("FileInvalid", this, f, [i])),
                        d(f.wrapper).find("input[type=file]").not(r).each(function() {
                            d.each(h(this), function(e, i) {
                                if (i.name) {
                                    var t = (i.name || "").replace(/^C:\\fakepath\\/gi, "");
                                    l != t && l != t.substr(t.length - l.length) || (o[o.length] = a(f.STRING.duplicate),
                                    f.trigger("FileDuplicate", r, f, [i]))
                                }
                            })
                        }),
                        0 < f.maxfile && 0 < t && t > f.maxfile && (o[o.length] = a(f.STRING.toobig),
                        f.trigger("FileTooBig", this, f, [i]));
                    ...
                    0 < f.max && a.length + l.length > f.max && (o[o.length] = f.STRING.toomany.replace("$max", f.max),
                    f.trigger("FileTooMany", this, f, n)),
                    0 < f.maxsize && c > f.maxsize && (o[o.length] = f.STRING.toomuch.replace("$size", g(c) + " > " + g(f.maxsize)),
                    f.trigger("FileTooMuch", "", f, n));
                    var p = d(f.clone).clone();
                    if (p.addClass("MultiFile"),
                    0 < o.length)
//                        return f.error(o.join("\n\n")),
                        return f.error(o.join("")),	//2020.08.05. juseong 개행 제거
                        f.n--,
                        f.addSlave(p[0], m),
                        u.parent().prepend(p),
                        u.remove(),
            ...
            f.addToList = function(c, e, i) {
                ...
                f.list.append(t.append(a, " ", o)),
                f.trigger("afterFileAppend", c, f, i),
                f.trigger("FileChange", c, f, f.files)
            }
            ,
    ...
    d.extend(d.fn.MultiFile, {
        ...
        reset: function() {
            ...
            return e && d(e.list).find("a.MultiFile-remove").click(),
            d(this)
        },
        files: function() {
            var e = this.MultiFile("data");
        ...
        intercept: function(e, i, t) {
            var a, l;
            if ((t = t || []).constructor.toString().indexOf("Array") < 0 && (t = [t]),
            "function" == typeof e)
                return d.fn.MultiFile.disableEmpty(),
